...
const API_KEY =
  process.env.NODE_ENV === "production"
    ? process.env.API_KEY
    : require("../apikey.json").API_KEY;

console.log("TCL: API_KEY", API_KEY);
const RIOT_URL = "https://kr.api.riotgames.com/";

export const getSummonerDTO = summonerName => {
  let REQUEST_URL =
    RIOT_URL + `/lol/summoner/v4/summoners/by-name/${summonerName}`;
...
export const getLeagueEntryDTO = encryptedSummonerId => {
  let REQUEST_URL =
    RIOT_URL + `/lol/league/v4/entries/by-summoner/${encryptedSummonerId}`;
  REQUEST_URL += `?api_key=${API_KEY}`;
...
export const getMatchlistDTO = (encryptedAccountId, from = 0, to = 20) => {
  let REQUEST_URL =
    RIOT_URL + `/lol/match/v4/matchlists/by-account/${encryptedAccountId}`;
